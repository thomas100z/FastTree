        ...
        for node in self.nodes:
            ...
            # if current node is leaf node A
                ...
                if node.get_sibling().is_leaf:
                    b = node.get_sibling()
                    a.branch_length = Distances.log_corrected_profile_distance(a.profile, b.profile)

                # if sibling node is internal node (calculate d(A,BC))
                else:
                    b = node.get_sibling().children[0]
                    c = node.get_sibling().children[1]
                    a.branch_length = (Distances.log_corrected_profile_distance(a.profile,
                                                                                b.profile) + Distances.log_corrected_profile_distance(
                        a.profile, c.profile) - \
                                       Distances.log_corrected_profile_distance(b.profile, c.profile)) / 2
            ...
            # if current node is internal node
                ...
                if not node.parent:
                    ...
                    if node.get_sibling().is_leaf:
                        c = node.get_sibling()
                        ab.branch_length = (Distances.log_corrected_profile_distance(a.profile, c.profile) +
                                            Distances.log_corrected_profile_distance(b.profile, c.profile) -
                    ...
                    # if sibling node is internal node CD, consisting of nodes C and D (calculate d(AB,CD))
                        ...
                        ab.branch_length = (Distances.log_corrected_profile_distance(a.profile,
                                                                                     c.profile) + Distances.log_corrected_profile_distance(
                            a.profile, d.profile) + \
                                            Distances.log_corrected_profile_distance(b.profile,
                                                                                     c.profile) + Distances.log_corrected_profile_distance(
                                    b.profile, d.profile)) / 4 \
                                           - (Distances.log_corrected_profile_distance(a.profile,
                                                                                       b.profile) + Distances.log_corrected_profile_distance(
                            c.profile, d.profile)) / 2
